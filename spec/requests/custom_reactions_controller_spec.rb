...
describe DiscourseMultiReactions::CustomReactionsController do
  ...
  context '#toggle' do
    ...
    it 'toggles reaction' do
      ...
      put "/discourse-multi-reactions/posts/#{post_1.id}/custom-reactions/thumbsup/toggle.json"
      expect(DiscourseMultiReactions::Reaction.count).to eq(1)
      expect(DiscourseMultiReactions::ReactionUser.count).to eq(1)
      expect(response.status).to eq(200)
      expect(response.parsed_body['reactions']).to eq(expected_payload)

      reaction = DiscourseMultiReactions::Reaction.last
      ...
      sign_in(user_1)
      put "/discourse-multi-reactions/posts/#{post_1.id}/custom-reactions/thumbsup/toggle.json"
      expect(DiscourseMultiReactions::Reaction.count).to eq(0)
      expect(DiscourseMultiReactions::ReactionUser.count).to eq(0)
      expect(response.status).to eq(200)
    ...
    it 'sends MessageBus message that user acted' do
      ...
      messages = MessageBus.track_publish("/topic/#{post_1.topic.id}") do
        put "/discourse-multi-reactions/posts/#{post_1.id}/custom-reactions/thumbsdown/toggle.json"
        put "/discourse-multi-reactions/posts/#{post_1.id}/custom-reactions/thumbsdown/toggle.json"
      end
    ...
    it 'errors when reaction is invalid' do
      sign_in(user_1)
      put "/discourse-multi-reactions/posts/#{post_1.id}/custom-reactions/invalid-reaction/toggle.json"
      expect(DiscourseMultiReactions::Reaction.count).to eq(0)
  ...
  context 'positive notifications' do
    ...
    it 'creates notification when first like' do
      sign_in(user_1)
      put "/discourse-multi-reactions/posts/#{post_1.id}/custom-reactions/heart/toggle.json"
  ...
  context 'reaction notifications' do
    it 'calls ReactinNotification service' do
      sign_in(user_1)
      DiscourseMultiReactions::ReactionNotification.any_instance.expects(:create).once
      DiscourseMultiReactions::ReactionNotification.any_instance.expects(:delete).once
      put "/discourse-multi-reactions/posts/#{post_1.id}/custom-reactions/thumbsdown/toggle.json"
      put "/discourse-multi-reactions/posts/#{post_1.id}/custom-reactions/thumbsdown/toggle.json"
    end
  end
